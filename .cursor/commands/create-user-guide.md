...
Phân tích giao diện web (menu, Views, JavaScript, thông báo) và tạo hoặc cập nhật tài liệu **Hướng dẫn sử dụng (HDSD)** dành cho người dùng cuối — theo cấu trúc chương số La Mã, mục đánh số và placeholder ảnh trong skill (`.cursor/skills/create-user-guide/template.md`). Mỗi màn hình: **tổng quan** và **thao tác đặc biệt**; bỏ qua tìm kiếm/lọc chuẩn. Không mô tả kỹ thuật (API, bảng DB, tên hàm); không có mục nhật ký thay đổi.
...
5. **Tạo hoặc cập nhật** `Docs/HDSD/HDSD_{Tên}.md`:
   ...
   - Mỗi màn hình menu = một chương (số La Mã, tiêu đề IN HOA) — **tổng quan** + **chức năng/thao tác đặc biệt** của màn hình
   - Chương **LƯU Ý KHI SỬ DỤNG** ở cuối
   - **Không** viết mục **NHẬT KÝ THAY ĐỔI**; khi **update** file cũ có nhật ký thì xóa chương đó
6. **Phạm vi nội dung mỗi màn hình** (xem chi tiết trong skill):
   - **Có:** tổng quan bố cục; ý nghĩa cột/trường khi cần hiểu nghiệp vụ; thao tác CRUD/xuất file/validation **riêng** của màn hình; bộ lọc hoặc tìm kiếm **chỉ khi** có quy tắc đặc biệt (giá trị mặc định bất thường, phụ thuộc combobox, auto-load, điều kiện bắt buộc, …).
   - **Bỏ qua:** tìm kiếm, lọc, lọc nhanh cột theo pattern chuẩn của hệ thống — không cần hướng dẫn từng bước nếu không khác biệt so với màn hình khác.
7. **Ngôn ngữ**: Toàn bộ nội dung HDSD và phản hồi cho user khi chạy command này bằng **tiếng Việt**.
8. **Kết thúc**: In tóm tắt đường dẫn file, số chương đã viết/cập nhật, và nhắc user dán ảnh chụp màn hình vào các placeholder.
...
Agent: Đọc menu và tất cả View, tạo `Docs/HDSD/HDSD_MyWebApp.md` với đủ chương (Giới thiệu, Mục lục, từng màn hình — tổng quan + thao tác đặc biệt, Lưu ý).

**Ví dụ 2 — Một vài màn hình**  
User: `/create-user-guide MyWebApp Danh mục kho, Báo cáo doanh thu`  
Agent: Chỉ viết/cập nhật các chương tương ứng và điều chỉnh mục lục.
...
Agent: Tìm `Docs/HDSD/HDSD_*.md`, so sánh với code hiện tại, cập nhật phần thay đổi; gỡ mục tìm kiếm/lọc cơ bản và chương Nhật ký nếu file cũ còn.
...
- Nếu project đã có `Docs/HDSD/HDSD_*.md`, đọc file đó khi **update** để giữ phong cách; khi **tạo mới** thì theo skill + template.
- Mỗi chương màn hình **ngắn gọn**: ưu tiên tổng quan và điểm khác biệt — không lặp lại quy trình tìm kiếm/lọc chuẩn.
