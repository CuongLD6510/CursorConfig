...
3. **Create or update doc file**: For each controller, write `API_[ControllerName].md` (e.g. `API_UserController.md`) in the agreed docs folder (e.g. `docs/` or `docs/APIs/`), following the format below.
...
6. **Output**: Save the file(s) and, if useful, print a one-line summary per file (path and controller name).
...
Agent: Reads `UserController.cs`, related services/models, then creates `docs/API_UserController.md` with I. TỔNG QUAN, II. MỤC LỤC, III. DANH SÁCH API (each endpoint as ## 1., ## 2., … with Input required, Input full, Output and cases), IV. CHANGED LOG.

**Example 2 — All controllers**  
User: `/create-api-doc-by-controller all`  
Agent: Finds all controllers in the project, then generates one `API_[ControllerName].md` per controller in the docs folder, each following the same structure.
...
- If the project already has a docs location (e.g. `docs/APIs/`), use it; otherwise use `docs/` or ask once where to put the files.
