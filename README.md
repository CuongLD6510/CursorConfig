...
- **Common config ("Cài đặt chung")**: Commands, rules, skills, and subagents that apply across many projects (e.g. `/task`, `/build`, `/commit`, global rules, create-rule/create-skill/create-subagent/migrate-to-skills/update-cursor-settings skills, code-reviewer and planner subagents). The bundle is **self-contained**: all skills live in `.cursor/skills/`; you do not need to run migrate-to-skills or depend on Cursor's built-in skills. Store them here as a template and optionally copy/symlink to `~/.cursor/` or to each project's `.cursor/`.
...
```
.cursor/
├── commands/     # Slash commands (.md): task, question, build, commit, create-config-for-this-project, etc.
├── rules/        # Rules (.mdc): global-rules, authoring-guidelines
├── skills/       # Skills (each in a subfolder with SKILL.md): create-rule, create-skill, create-subagent, migrate-to-skills, update-cursor-settings, task, question, build, commit, create-config-for-this-project, create-api-doc-by-controller
└── agents/       # Subagents (.md): code-reviewer, planner, etc.
...
4. **Use the commands**  
   ...
   - `/commit` — Propose a commit message in a code block (`[TYPE]: [Message]`).
